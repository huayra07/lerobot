...
def rollout(
    ...
    while not np.all(done) and step < max_steps:
        ...
        # ---- DEBUG PROBE (only first step) ----
        DEBUG_ONCE = (step == 0)

        if DEBUG_ONCE:
            print("[OBS_KEYS]", sorted(list(observation.keys())))
            print("[HAS_LANG_EMB]", "language_embedding" in observation)
            if "language_embedding" in observation:
                le = observation["language_embedding"]
                if torch.is_tensor(le):
                    print("[LANG_EMB] shape", tuple(le.shape), "min/max", float(le.min()), float(le.max()))
                else:
                    print("[LANG_EMB] type", type(le))

        with torch.inference_mode():
            action_raw = policy.select_action(observation)

        if DEBUG_ONCE:
            ar = action_raw
            if torch.is_tensor(ar):
                print("[ACTION_RAW] shape", tuple(ar.shape), "min/max", float(ar.min()), float(ar.max()))
            else:
                print("[ACTION_RAW] type", type(ar))

        action_post = postprocessor(action_raw)

        if DEBUG_ONCE:
            ap = action_post
            print("[ACTION_POST] shape", tuple(ap.shape), "min/max", float(ap.min()), float(ap.max()))

        action_transition = {ACTION: action_post}
        action_transition = env_postprocessor(action_transition)
        action_env = action_transition[ACTION]

        if DEBUG_ONCE:
            ae = action_env
            print("[ACTION_ENV] shape", tuple(ae.shape), "min/max", float(ae.min()), float(ae.max()))
        # ---- END DEBUG PROBE ----
