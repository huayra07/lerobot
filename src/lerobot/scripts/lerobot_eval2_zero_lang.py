...
from examples.training.train_pusht_lang_diffusion_STEP_C_FIXED2 import (
    CLIPLanguageEncoder, HybridCLIPDiffusionPolicy
)
import os
DEBUG_ZERO_LANG = os.environ.get("ZERO_LANG", "0") == "1"
...
def rollout(
    ...
    while not np.all(done) and step < max_steps:
        ...
        if isinstance(observation, dict) and "language_embedding" in observation:
            ...
            print("[TEST] zeroed language_embedding")
        if DEBUG_ZERO_LANG:
                observation["language_embedding"] = torch.zeros_like(observation["language_embedding"])
                print("[TEST] zeroed language_embedding in observation")
