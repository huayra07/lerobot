#!/usr/bin/env python3
# ==============================================================================
# FILE PATH: examples/training/train_pusht_lang_diffusion_STEP_C_FIXED_huggingface.py
# ==============================================================================
"""
...
def _is_hf_id(s: str) -> bool:
    # minimal check: "user/repo" and no spaces
    return ("/" in s) and (" " not in s) and (len(s.split("/")) == 2)

def _hf_pull_folder(repo_id: str, subdir: str, revision: str | None = None) -> Path:
    # downloads the whole repo (or revision) then points into subdir
    ...
    p = Path(local_root) / subdir
    if not p.exists():
        raise FileNotFoundError(f"HF resume subdir not found: {repo_id}:{subdir} -> {p}")
    return p
...
def main() -> None:
    ...
    hf_revision = os.environ.get("HF_REVISION", "").strip() or None
